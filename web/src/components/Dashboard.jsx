...
const Dashboard = ({ user }) => {
  ...
  return (
    <div className={`max-w-7xl mx-auto space-y-8 pb-10 animate-in fade-in duration-500 ${isAdmin ? 'text-slate-200' : 'text-slate-800 dark:text-slate-200'}`}>

      {/* Top Header */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className={`text-3xl font-black ${isAdmin ? 'text-white' : 'text-slate-800 dark:text-white'}`}>
            {isAdmin ? "Platform Control" : "Kumusta"}, <span className="text-[#16A394]">{user?.name?.split(' ')[0] || "Admin"}</span>!
          </h1>
          <p className={isAdmin ? 'text-slate-400 font-medium' : 'text-slate-500 dark:text-slate-400 font-medium'}>
      ...
      <div className={`${isAdmin ? 'bg-slate-900 border border-white/5' : 'bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800'} p-8 rounded-[3rem] shadow-xl space-y-6`}>
        <div className="flex justify-between items-end">
          <div>
            <h3 className={`text-xl font-bold ${isAdmin ? 'text-white' : 'text-slate-800 dark:text-white'}`}>
        ...
        <div className="w-full mt-4">
          <ResponsiveContainer width="100%" height={300}>
            <AreaChart data={(isAdmin ? platformStats.weeklySales : stats.weeklySales) || []}>
              ...
              <CartesianGrid strokeDasharray="3 3" vertical={false} stroke={isAdmin ? "#1e293b" : (document.documentElement.classList.contains('dark') ? "#1e293b" : "#f1f5f9")} />
              <XAxis 
                dataKey="day" 
                axisLine={false} 
                tickLine={false} 
              ...
              <Tooltip 
                contentStyle={{
                  ...
                  backgroundColor: isAdmin || document.documentElement.classList.contains('dark') ? '#0f172a' : '#ffffff',
                  color: isAdmin || document.documentElement.classList.contains('dark') ? '#f8fafc' : '#1e293b'
                }}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className={`${isAdmin ? 'bg-slate-900/50 border border-white/5' : 'bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800'} rounded-[3rem] shadow-xl overflow-hidden`}>
            <div className={`divide-y ${isAdmin ? 'divide-white/5' : 'divide-slate-50 dark:divide-slate-800'}`}>
              {((isAdmin ? platformStats.recentTransactions : stats.recentTransactions) || []).length === 0 ? (
                ...
                <div key={txn.id} className={`p-6 flex items-center justify-between transition-colors cursor-pointer group ${isAdmin ? 'hover:bg-white/5' : 'hover:bg-slate-50 dark:hover:bg-slate-800'}`}>
                  <div className="flex items-center gap-4">
                    <div className={`h-12 w-12 rounded-2xl flex items-center justify-center font-bold transition-colors ${isAdmin ? 'bg-slate-800 text-teal-400' : 'bg-slate-50 dark:bg-slate-800 text-[#16A394]'}`}>
                      #{txn.id}
                    </div>
                    <div>
                      <p className={`font-bold ${isAdmin ? 'text-slate-200' : 'text-slate-800 dark:text-slate-200'}`}>
                  ...
                  <div className="text-right">
                    <p className={`font-black ${isAdmin ? 'text-white' : 'text-slate-800 dark:text-white'}`}>₱{(txn.totalAmount || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
        ...
        <div className="space-y-4">
          <h3 className={`text-xl font-bold px-2 ${isAdmin ? 'text-white' : 'text-slate-800 dark:text-white'}`}>
            {isAdmin ? "Top Vendors" : "Top Selling Items"}
          </h3>
          <div className={`${isAdmin ? 'bg-slate-900/50 border border-white/5' : 'bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800'} p-8 rounded-[3rem] shadow-xl space-y-6`}>
            {isAdmin ? (
            ...
            <button
              onClick={() => navigate(isAdmin ? '/admin/vendors' : '/inventory')}
              className={`w-full py-4 mt-4 border-2 border-dashed rounded-2xl font-bold text-sm transition-all flex items-center justify-center gap-2 ${isAdmin ? 'border-white/10 text-slate-500 hover:border-teal-400 hover:text-teal-400' : 'border-slate-200 dark:border-slate-700 text-slate-400 dark:text-slate-500 hover:border-[#16A394] hover:text-[#16A394]'
                }`}>
