    ...
    <div className="space-y-8 animate-in fade-in duration-500 text-slate-200">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 mt-1">Control and monitor all registered sari-sari stores on your platform.</p>
        </div>
        <div className="flex items-center gap-3">
          <button className="px-4 py-2.5 bg-slate-900 border border-slate-800 text-slate-300 rounded-xl font-semibold shadow-sm hover:bg-slate-800 transition-all flex items-center gap-2 border-white/5">
            <Filter size={18} />
            Filter
          </button>
      ...
      <div className="bg-slate-900/40 backdrop-blur-xl rounded-[2.5rem] border border-white/5 shadow-2xl overflow-hidden">
        <div className="p-6 border-b border-white/5 flex items-center justify-between gap-4 bg-white/5">
          <div className="relative flex-1 max-w-md">
            ...
            <input 
              ...
              className="w-full pl-12 pr-4 py-3 bg-slate-950/50 border border-white/5 rounded-2xl focus:ring-2 focus:ring-teal-500/20 transition-all text-sm text-slate-200 placeholder:text-slate-600"
            />
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-white/5">
              {loading ? (
                <tr><td colSpan="5" className="text-center py-10 text-slate-500">Loading platform vendors...</td></tr>
              ) : displayVendors.map((vendor) => (
                <tr key={vendor.id} className="hover:bg-white/5 transition-colors group">
                  <td className="px-6 py-5">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-xl bg-slate-800 flex items-center justify-center text-slate-400 font-bold border border-white/5 group-hover:border-teal-500/30 transition-all">
        ...
        <div className="p-6 bg-white/5 border-t border-white/5 text-center">
          <button className="text-sm font-bold text-teal-400 hover:text-teal-300 transition-colors">
